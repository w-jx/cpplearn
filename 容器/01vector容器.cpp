...
//打印vector
template<typename T>
void printV(vector<T>& v,string str="print vector")
{	
	cout << str<<endl;
	for (auto iter : v) {
		std::cout << iter << " ";
...
void test01()
{
	...
	vector<int>::iterator vEnd = v.end();//尾逾迭代器
	...
	//	std::cout << *a << std::endl;
	//}
	for (auto iter = v.rbegin(); iter != v.rend(); ++iter) {//反向遍历，迭代器也是++
		cout << *iter << endl;
	}
}
//vector存放自定义结构类型
...
void test05()//
{
	...
	vector<int> vfirst{ 1,3,5,7,9 };
	vector<int> vsecond{ 2,4,6,8,10 };
	vfirst.reserve(vfirst.size() + vsecond.size());
	vfirst.insert(vfirst.end(), vsecond.begin(), vsecond.end());
	for_each(vfirst.begin(), vfirst.end(), myPrint);
...
void test07()
{
	...
	v1.insert(iter, 10);
	printV(v1,"在迭代器指定位置添加元素");

	iter = v1.begin();//迭代器使用过以后要重新指定
	iter++;
	v1.insert(iter, 3, 9);//方式2,在迭代器指定位置添加 指定数量的元素,参数2为数量
	printV(v1,"在迭代器指定位置添加指定数量的元素");

	/* 删除 */
	iter = v1.begin();
	auto iter_end = iter + 3;
	v1.erase(iter, iter_end);//删除 两个迭代器之间的所有元素,注意:不包括最后一个
	printV(v1,"删除两个迭代器之间的元素");

	v1.erase(v1.begin() );
	printV(v1, "删除指定迭代器对应的元素");
...
void test11()
{
	...
	for_each(v.begin(), v.end(), [](const auto& iter) {cout << iter << " "; });
...
void test13()
{
	vector<int> vr{ 100,200,300,400,500 };
	for (vector<int>::iterator it = vr.begin(); 
		it != vr.end(); 
		it++) {
		if (*it == 300)
			vr.erase(it);
